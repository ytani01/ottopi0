...
from ottopi0.jrpcclnt.jrpcclnt_bt import JrpcClntBt, PiBtInput
...
class TestJrpcClntBt:
    ...
    def mock_deps(self):
        ...
        """
        with patch("ottopi0.jrpcclnt.jrpcclnt_bt.PiBtInput") as MockPiBtInput, \
             patch("ottopi0.jrpcclnt.jrpcclnt_bt.JrpcClient") as MockJrpcClient, \
    ...
    def test_cb_ev_single_key(self, mock_deps):
        ...
        # Mock key event
        key_name = "KEY_A"
        key_state = 1 # down (not handled specifically in logic, but checks keys["up"]/["hold"])
        # PiBtInput.KEY["up"] is typically 0, "hold" is 2. Let's assume 1 is down.
        # We need to ensure we don't hit "up" or "hold" returns.
        # Need to know values of PiBtInput.KEY. 
        # Since we mocked PiBtInput, checking the class attribute access in code:
    ...
    def test_cb_ev_chord(self, mock_deps):
        ...
        keys_config = {
            "KEY_A": "cmd_a",
            "KEY_B": "cmd_b",
            "KEY_A-KEY_B": "cmd_ab" # Normalized key in config implies code will normalize it too if written as B-A
        }
        ...
        clnt.cb_ev("KEY_B", 1, onkeys) # Triggered by B down, while A is already down

...
from ottopi0.func_servo import Servo
...
def mock_pi0servo_classes(mocker):
    ...
    mock_str_cmd_to_json = mocker.patch("ottopi0.func_servo.StrCmdToJson", spec=True)
    mock_json_rpc_worker = mocker.patch("ottopi0.func_servo.JsonRpcWorker", spec=True)
    return mock_str_cmd_to_json, mock_json_rpc_worker

@pytest.fixture
def mock_logger(mocker):
    """Fixture to mock the get_logger function."""
    return mocker.patch("ottopi0.func_servo.get_logger", return_value=MagicMock())
...
class TestServo:
    ...
    def test_init(self, mock_pi0servo_classes, mock_logger):
        ...
        servo = Servo(mock_pi, self.PINS, self.ANGLE_FACTORS, debug=debug_mode)

        StrCmdToJson.assert_called_once_with(self.ANGLE_FACTORS, debug=debug_mode)
        JsonRpcWorker.assert_called_once_with(mock_pi, self.PINS, debug=debug_mode)
    ...
    def test_call(self, mock_pi0servo_classes, mock_logger):
        ...
        servo.parser.cmdstr_to_jsonliststr.return_value = expected_json_list_str
